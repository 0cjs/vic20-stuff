...
- `vic_src`: "Converted to VAX/VMS BSO format 10/30/87 by Fred Bowen."
  Kinda looks like it might be original Commodore source, what with
  all the copyright notices and terrible label names and comments.
  Includes both `.src` and `.lst` files; the latter are handy because
  it makes it very easy to see both names and addresses for any
  particular line of code. (From [zim-vic20] `vic_src.tar.gz`.)
