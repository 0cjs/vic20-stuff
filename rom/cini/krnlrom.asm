...
;	(c) 1995-2016 Richard A. Cini, Jr. All Rights Reserved.			
;				
...
;====================================================================				
; POLYX - Function Series Evaluation $E040				
;				
POLYX				
	STA CSBFLN			
...
;====================================================================				
; RMULC - Constant Manipulation: RND Data $E08A				
;				
RMULC				
...
;
; Kernal patch routines for BASIC $E0F6-E260
;
;====================================================================				
...
;====================================================================				
; COS - Perform COS function/BASIC $E261				
...
;====================================================================				
; SIN - Perform SIN function/BASIC $E268				
...
;====================================================================				
; TAN - Perform TAN function/BASIC $E2B1				
...
;====================================================================				
; ATN - Perform ATN function/BASIC $E30B				
;	On entry, FAC1 contains radians. On exit, FAC1 holds TAN.			
;				
ATN				
	LDA FC1SGN			;sign
...
;====================================================================				
; INITAT - CHRGET to be copied to zero-page $E387				
;				
INITAT				
	INC TXTPTR			
	BNE ININCR			
...
;====================================================================				
; INITCZ - Initialize BASIC RAM	$E3A4		 	
;				
INITCZ				
...
FREMES				
	.db " BYTES FREE", $0D, $00	;$E429	

WORDS				
	.db $93, "**** CBM BASIC V2 ****", $0D, $00			

;====================================================================				
; BVTRS - Basic vector table $E44F				
;				
BVTRS				
	.dw ERROR+3			;$C43A
	.dw MAIN+3			;$C483
...
;====================================================================				
; FIXUP1 - Absolute location fixup area #1 $E47C				
;				
FIXUP1		
	.dw $DFC4			;0 e47c
	.dw RMULC			;2 7e
...
;====================================================================				
; SOUT1 - Serial output 1 (inverted logic) $EA40			
;				
SOUT1				
	LDA D2PCR			;load PCR
...
;====================================================================				
; SOUT0- Serial output 0 (inverted logic) $E4A9				
;				
SOUT0				
	LDA D2PCR			;load PCR
...
;====================================================================				
; TPWPCH - Tape write patch for CLOSE $E4CF				
;				
TPWPCH				
...
;====================================================================				
; FIXUP2 - Absolute location fixup area #2 $E4DA				
;				
FIXUP2	;$E4DA			
...
;====================================================================				
; IIOBASE - Return I/O base address $E500				
...
;====================================================================				
; ISCREN - Return screen organization $E505				
...
;====================================================================				
; IPLOT - Read/set cursor position $E50A				
...
;====================================================================				
; PANIC - Set I/O defaults (unused entry point??) $E5B5				
;				
PANIC				
	JSR IODEF1			;reset devices and VIC regs
	JMP HOME			;home cursor
;				
; Real PANIC entry; reset default devices $E5BB				
;				
IODEF1					
	LDA #$03				
...
;====================================================================				
; LP2 - Input from keyboard (prompt loop) $E5CF				
;				
LP2				
...
;====================================================================				
; INKEY - Input from keyboard (keyboard prompt loop) $E5E5				
;				
INKEY				
...
;====================================================================				
; INSCR - Input from screen/keyboard until <RET> $E64F				
;				
INSCR				
	TYA			
	PHA			
...
;====================================================================				
; CMD4 cmd- Setup screen print $E6C5				
;				
CMD4				
	ORA #%01000000			;$40
	LDX RVS			
...
;====================================================================				
; CSLPLS - Advance cursor (calc MSB for line start) $E6EA				
;				
CSRPLS				
	JSR CKLNPL			
	INC CSRIDX			
...
;====================================================================				
; CKLNMI - Check line decrement	$E8E8			
;				
CKLNMI				
...
;====================================================================				
; CKLNPL - Check for line increment $E8FA				
;				
CKLNPL				
	LDX #$04			;loop counter
	LDA #$15			;column 15
...
;====================================================================	
; SCRLSC - Scroll screen evaluation $E975	
;	
SCRLSC	
	LDA SAL
	PHA
...
;====================================================================				
; INSSPC - Open a space on the screen (insert a second line) $E9EE				
;				
INSSPC				
	LDX CURROW			
...
;====================================================================	
; MOVELN - Move a screen line up $EA56	
;				
MOVELN				
	AND #%00000011			;$03
	ORA HIPAGE			
...
;====================================================================				
; COPYCL - Synchronize color transfer $EA6E				
;				
COPYCL				
...
;====================================================================				
; CCOLRAM - Calculate pointer to color RAM $EAB2				
;				
CCOLRAM				
...
;====================================================================				
; KDECOD - Pointers to keyboard decode tables $EC46				
;				
KDECOD				
...
;	
;VIC chip video control constants $EDE4	
VICSUP	
...
;====================================================================				
; ITALK - Send TALK command on serial bus (internal) $EE14				
...
;====================================================================				
; ILISTN - Send LISTEN command on serial bus (internal)	$EE17			
;	On entry, .A is a device number from 0 to 30			
; 				
ILISTN				
...
;====================================================================				
; ISECND - Send LISTEN secondary address (internal) $EEC0				
...
;====================================================================				
; ITKSA - Sent TALK secondary address (internal) $EECE				
;	On entry, .A is secondary address from 0 to 31.			
...
;====================================================================				
; ICIOUT - Send serial deferred (internal) $EEE4				
...
;====================================================================				
; IUNTLK - Send UNTALK on serial bus (internal) $EEF6			
;	On entry, UNKNOWN			
;	Required prior calls:  FFB4/TALK			
;				
IUNTLK				
...
;====================================================================				
; IUNLSN - Send UNLISTEN on serial bus (internal) $EF04				
;	On entry, UNKNOWN			
;	Required prior calls:  FFB1/LISTEN			
;				
IUNLSN				
...
;====================================================================				
; IACPTR - Receive from serial bus (internal) $EF19				
;	On entry, UNKNOWN			
...
;====================================================================				
; SCLK1 - Set Serial CLK "1" (inverted logic) $EF84				
;				
SCLK1				
	LDA D2PCR			;PCR
...
;====================================================================				
; SCLK0 - Set Serial CLK "0" (inverted)	$EF8D			
;				
SCLK0				
	LDA D2PCR			;PCR
...
;====================================================================				
; SSNDN - Send new RS-232 byte $EFEE				
;				
SSNDN				
	LDA M51CDR			
	LSR A			
...
;====================================================================				
; BITCNT - Compute RS-232 bit count $F027				
;				
BITCNT				
	LDX #$09			;8+1 bits
	LDA #$20			
...
;====================================================================	
; PROCSB - Process RS-232 byte $F068 	
;	
PROCSB	
	LDA INBIT			
	BNE RXSET			
...
;====================================================================				
; SSENDB - Output to RS-232 buffer $F0ED				
;				
SSENDB				
	LDY RODBE			
	INY			
...
;====================================================================				
; SBIDLE - Set timer for serial bus timeout $F160				
;				
SBIDLE				
	PHA				;save .A
	LDA D1IER			;get IER
...
;====================================================================					
; KIOMSG - KERNEL I/O messages $F174					
;					
KIOMSG					
...
;====================================================================				
; DIRMSG - Print message to screen if direct mode $F1E2				
...
;====================================================================				
; IGETIN - Get a byte (internal) $F1F5				
...
;====================================================================				
; ICHRIN - Input a byte (internal) $F20E				
;	On entry, UNKNOWN			
...
;====================================================================				
; GTTSCH - Get input from tape/serial/RS-232 (main device input) $F250				
;				
GTTSCH				
...
;====================================================================				
; ICHROT - Output one character (internal) $F27A				
...
;====================================================================				
; ICHKIN - Set input device (internal) $F2C7			
...
;====================================================================				
; ICHKOT - Set output device (internal) $F309				
;	On entry, .X is file number			
...
;====================================================================				
; ICLOSE - Close file (internal) $F34A				
...
;====================================================================				
; ICLALL - Abort all files (internal) $F3EF				
;	On entry, NONE			
;				
ICLALL				
	LDA #$00			
...
;====================================================================				
; ICLRCH - Restore default I/O (internal) $F3F3				
;	On entry, NONE			
;				
ICLRCH				
	LDX #$03			
...
;====================================================================				
; ILOAD - Load RAM from device (internal) $F542				
...
;====================================================================				
; LNKLOD - Load RAM (indirect from $0330) $F549				
;				
LNKLOD				
...
;====================================================================				
; TAPLOA - Load file from tape $F5CA				
;				
TAPLOA				
	CMP #$02			;RS232?
...
;====================================================================				
; LNKSAV - Link to SAVE (indirect from $0302) $F685				
;				
LNKSAV				
...
;====================================================================				
; IUDTIM - Update RTC (internal) $F734				
;	Called by IRQ			
;				
IUDTIM				
	LDX #$00			
...
;				
IOERMS1				
	LDA #$01			;Too Many Files $F77E
	.db $2C			

IOERMS2				
	LDA #$02			;File Already Open $F781
	.db $2C 			

IOERMS3				
	LDA #$03			;File Not Open $F784
	.db $2C 			

IOERMS4				
	LDA #$04			;File Not Found $F787
	.db $2C 			

IOERMS5				
	LDA #$05			;Device Not Present $F78A
	.db $2C 			

IOERMS6				
	LDA #$06			;Not Input File $F78D
	.db $2C 			

IOERMS7				
	LDA #$07			;Not Output File $F790
	.db $2C 			

IOERMS8				
	LDA #$08			;Missing File Name $F793
	.db $2C 			

IOERMS9				
	LDA #$09			;Illegal Device Number $F796
...
;====================================================================
; LOCTPH - Read any tape header $F7AF
...
;====================================================================				
; GETBFA - Get start of tape buffer $F84D				
...
;====================================================================				
; SETBST - Set buffer start/end pointers $F854				
;				
SETBST				
	JSR GETBFA			;get buffer address
	TXA			
...
;====================================================================				
; LOCSPH- Find specific tape header $F867				
;				
LOCSPH				
	JSR LOCTPH			;search for next header
...
;====================================================================				
; PLAYMS - Wait for tape key on read $F894				
;				
PLAYMS				
...
;====================================================================				
; TPSTAT - Check tape key status $F8AB				
;				
TPSTAT				
	LDA #%01000000			;$40
...
;====================================================================				
; RECDMS - Wait for tape key on write $F8B7				
;				
RECDMS				
...
;====================================================================				
; TPWRIT - Initiate tape write $F8E3				
;				
TPWRIT				
...
;====================================================================				
; TPSTOP - Check for tape stop $F94B				
;				
TPSTOP				
...
;====================================================================	
; RDTPBT - Read tape bits IRQ routine $F98E	
;	
RDTPBT	
	LDX D2TM2H
	LDY #$FF
...
;====================================================================				
; STORTC - Store tape characters $FAAD				
;				
STORTC				
	JSR NCHAR			
	STA BYTINF			
...
;====================================================================				
; RSTTPP - Reset tape pointer $FBD2				
;				
RSTTPP				
	LDA STAL+1			
	STA SAL+1			
...
;====================================================================	
; NCHAR - Set bit counter for new character (serial output) $FBDB	
;	
NCHAR	
	LDA #$08
	STA SBITCF
...
;====================================================================				
; TONE - Write one bit to tape (send tone to tape) $FBEA				
;				
TONE				
	LDA ROPRTY			;get bit
...
;====================================================================					
; TWRDTA - Write data to tape $FC06				
;					
TWRDTA					
	SEC				
	ROR SAL+1				
...
WRLDR2				
	...
	BNE TWRD3A			;$FC54
;				
; RESIRQ - Restore normal IRQ $FCCF				
;				
RESIRQ				
	PHP			
	SEI				;kill interrupts
...
;====================================================================				
; INCRDP - Bump read/write pointer $FD1B				
;				
INCRDP				
	INC SAL			
	BNE INCRSK			;$FD21
...
;====================================================================				
; SCNROM - Scan ROM areas for Autostart ROM signature $FD3F				
;				
SCNROM				
...
;====================================================================				
; IRESTR - Restore KERNEL hardware vectors (internal) $FD52				
;	On entry, NONE			
;				
IRESTR				
	LDX #$EA 			;FIXUP2;#$6D
...
;				
KNRLSV
	.dw IRQVEC, WARMST, LNKNMI, IOPEN	;$FD6D			
	.dw ICLOSE, ICHKIN, ICHKOT, ICLRCH	;$FD75		
	.dw ICHRIN, ICHROT, ISTOP,  IGETIN	;$FD7D		
	.dw ICLALL, WARMST, LNKLOD, LNKSAV	;FD85		

;====================================================================				
; RAMTAS - Initialize system contents $FD8D			
;				
RAMTAS				
	LDA #$00			
	TAX			
...
;====================================================================				
; IOINIT - Initialize I/O registers $FDF9				
;				
IOINIT				
	LDA #%01111111			;$7F
	STA D1IER			
...
;====================================================================				
; ISETNM - Set filename (internal) $FE49				
...
;====================================================================				
; ISETLF - Set logical file parameters (internal) $FE50				
...
;====================================================================				
; IRDST - Get I/O status word (internal) $FE57				
;				
IRDST				
	LDA CHANNL			;get device
	CMP #$02			;RS232?
...
;====================================================================				
; ISETMS - Control OS messages (internal) $FE66				
...
;====================================================================				
; ISETTO - Set IEEE timeout (internal) $FE6F				
...
;====================================================================				
; IMEMTP - Set/read top of memory (internal) $FE73				
...
;====================================================================				
; IMEMBT - Set/read bottom of memory  (internal) $Fe82				
;	On entry, SEC to read, .X/.Y is LSB/MSB			
;	          CLC to set, .X/.Y is LSB/MSB			
;				
IMEMBT				
...
;====================================================================
; WARMST - Default USER vector $FED2
...
;				
; EOI - End of Interrupt external entry point $FF56				
;				
EOI				
	PLA				;restore registers
	TAY			
...
IRQ				
	...
	JMP (BRKVP)			;jump to breakpoint through
					; vector which is WARMST @ $FED2
BRKSKIP					
	JMP (IRQVP)			;jump to IRQ through
					; vector which is IRQVEC @ $EABF
...
;====================================================================				
; - Power-on Vectors $FFFA/$FFFC/$FFFE				
;				
	.dw NMI				;$FEA9
	.dw RESET			;$FD22
	.dw IRQ				;$FF72
